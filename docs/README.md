# WFA-SQLite Documentation

Welcome to the central documentation hub for WFA-SQLite. This repository contains all architectural, API, security, and operational documentation, structured cleanly into distinct domains.

## System Architecture
* [System Overview](./system-architecture/SYSTEM_OVERVIEW.md)
* [How the System Works](./system-architecture/HOW_THE_SYSTEM_WORKS.md)
* [System Data Flow](./system-architecture/SYSTEM_DATA_FLOW.md)
* [Request Lifecycle](./system-architecture/REQUEST_LIFECYCLE.md)

## Backend
* [Backend Architecture](./backend/BACKEND_ARCHITECTURE.md)
* [Backend Patterns](./backend/BACKEND_PATTERNS.md)

## Frontend
* [Frontend Architecture](./frontend/FRONTEND_ARCHITECTURE.md)

## Database
* [Database Architecture](./database/DATABASE_ARCHITECTURE.md)
* [Database Guide](./database/DATABASE_GUIDE.md)
* [Indexes Guide](./database/INDEXES.md)

## API
* [API Overview](./api/API_OVERVIEW.md)
* [Authentication API](./api/AUTHENTICATION_API.md)
* [Employee API](./api/EMPLOYEE_API.md)
* [Reports API](./api/REPORTS_API.md)

## Authentication & Security
* [Security Overview](./security/SECURITY.md)

## Compliance & Legal
* [Data Classification Policy](./compliance/DATA_CLASSIFICATION.md)
* [Internal Privacy Policy](./compliance/PRIVACY_POLICY.md)
* [Terms of Use](./compliance/TERMS_OF_USE.md)

## Tests
* [Testing Strategy](./tests/TESTING_STRATEGY.md)

## Performance
* [Database Optimization](./performance/DATABASE_OPTIMIZATION.md)
* [Load Testing](./performance/LOAD_TESTING.md)

## Features
* [Workforce Management](./features/WORKFORCE_MANAGEMENT.md)

## Development
* [Local Setup](./development/LOCAL_SETUP.md)
* [Development Guidelines](./development/DEVELOPMENT_GUIDELINES.md)
* [Git Workflow](./development/GIT_WORKFLOW.md)

## Deployment
* [Deployment](./deployment/DEPLOYMENT.md)
* [Environment Configuration](./deployment/ENVIRONMENT_CONFIGURATION.md)
* [Health Checks](./deployment/HEALTH_CHECKS.md)

## ADR
* [Architecture Decisions](./adr/)
