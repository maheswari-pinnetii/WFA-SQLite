...
export const seedAnalytics = (db: Database, orgId: string) => {
  // Seed skills
  const skillCount = (db.prepare('SELECT COUNT(*) as count FROM skills').get() as any).count;
  if (skillCount === 0) {
  ...
  const performanceCount = (db.prepare('SELECT COUNT(*) as count FROM performancerecords').get() as any).count;
