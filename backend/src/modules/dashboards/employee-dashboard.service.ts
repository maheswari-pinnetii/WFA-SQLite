  ...
  async getDashboardData(user: any) {
    ...
    try {
      const balanceRows = await query(`
        SELECT totalDays as allocated, usedDays as used FROM leave_balances WHERE employeeId = ? AND companyId = ?
      `, [employeeId, orgId]);
    ...
    try {
      ...
      coreHours = coreHoursRows.reduce((acc: number, row: any) => {
        if (row.checkInTime && row.checkOutTime) {
          const inTime = new Date(row.checkInTime).getTime();
          const outTime = new Date(row.checkOutTime).getTime();
    ...
    try {
      const ticketsRows = await query(`
        SELECT 
          (SELECT COUNT(*) FROM leaverequests WHERE employeeId = ? AND organizationId = ? AND status = 'PENDING') +
          (SELECT COUNT(*) FROM correctionrequests WHERE employeeId = ? AND organizationId = ? AND status = 'PENDING') as count
    ...
    const kpis = {
      ...
      upcomingLeave: leaveBalance, // using leaveBalance
    ...
    const weeklyHours = attendance.slice(0, 5).map((a: any) => {
      let hours = 0;
      if (a.checkInTime && a.checkOutTime) {
