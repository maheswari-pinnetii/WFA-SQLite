  ...
  return (
    <DashboardErrorBoundary dashboardName="Admin Dashboard">
      <RoleGuard allowedRoles={[Role.ADMIN]} requiredPermission={Permission.SYSTEM_CONFIG}>
        ...
        {/* KPI metrics (10 KPIs) */}
        <KPIGrid>
          <KPICard title="Total Employees" value={analytics.data?.metrics?.totalEmployees ?? 0} icon={<Users size={20} />} color="emerald" trend="up" trendValue="12.4%" subtitle="Organization workforce" />
          ...
          <KPICard title="Attendance Rate" value={analytics.data?.metrics?.attendanceRate ?? '0%'} icon={<CalendarClock size={20} />} color="emerald" trend="up" trendValue="1.5%" subtitle="Organization average" />
          ...
          <KPICard title="Teams" value={analytics.data?.metrics?.teams ?? 0} icon={<Briefcase size={20} />} color="neutral" subtitle="Active teams" />
          <KPICard title="Pending Approvals" value={analytics.data?.metrics?.pendingApprovals ?? 0} icon={<Filter size={20} />} color="warning" subtitle="Requires attention" />
          <KPICard title="Open Leave Requests" value={analytics.data?.metrics?.openLeaveRequests ?? 0} icon={<CalendarOff size={20} />} color="warning" subtitle="Pending review" />
          
          <KPICard title="Attendance Exceptions" value={analytics.data?.metrics?.attendanceExceptions ?? 0} icon={<Clock size={20} />} color="danger" trend="down" trendValue="0.8%" subtitle="Late or missing" />
          <KPICard title="Open Tasks" value={analytics.data?.metrics?.openTasks ?? 0} icon={<FileSpreadsheet size={20} />} color="info" subtitle="Active workload" />
        </KPIGrid>
