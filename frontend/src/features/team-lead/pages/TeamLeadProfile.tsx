...
import { Users, Settings, Bell, Zap } from 'lucide-react';
...
export const TeamLeadProfile: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'squad' | 'security' | 'notifications'>('squad');

  return (
    <div className="space-y-6 max-w-4xl mx-auto animate-fadeIn pb-24">
      <ProfileHeader title="Team Lead Profile" subtitle="Manage your credentials, squad access, and notifications" />

      <div className="glass-panel p-6 md:p-8 space-y-6">
        {/* Tab Navigation */}
        <div className="flex flex-wrap gap-4 border-b border-[var(--border-color)]">
          {[
            { id: 'squad', label: 'Squad Access', icon: <Users size={16} /> },
        ...
        {activeTab === 'squad' && (
          <div className="p-6 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-center">
            <Zap size={32} className="mx-auto text-emerald-500 mb-2" />
            <h3 className="font-bold text-lg text-emerald-500">Squad Management</h3>
            <p className="text-xs text-slate-400 mt-1">You have privileges to manage and oversee your squad's daily attendance and tasks.</p>
          </div>
        )}
